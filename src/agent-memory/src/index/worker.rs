...
impl IndexWorker {
    ...
    pub fn spawn(
        ...
        let handle = thread::Builder::new()
            ...
            .spawn(move || {
                if let Err(e) = run_watcher(mount_clone, store_clone, emb_clone, cancel_rx, ready_tx) {
...
fn run_watcher(
    ...
    loop {
        ...
        if !pending_modify.is_empty() || !pending_remove.is_empty() {
            flush(&mount, &store, embedding.as_deref(), &mut pending_modify, &mut pending_remove)?;
        }
    }
...
fn flush(
    ...
    let rt_handle = embedding
        .and_then(|_| tokio::runtime::Handle::try_current().ok());
    ...
    for path in pending_modify.drain() {
        ...
        if let Some(rel) = relative(mount, &path) {
            ...
            if let (Some(emb), Some(rt)) = (embedding, rt_handle.as_ref()) {
                if let Ok(embedding_vec) =
                    rt.block_on(emb.embed(&body))
                {
